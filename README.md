...
- **Status island instead of banners and toasts.** The card on the map is the dog's status *and* the notification surface: a notification opens it into the full message with its action, and it closes after a few seconds or when swiped up. Its details list the dogs this phone is paired with (switch, or 連接其他機器狗). **Portrait teleop has no sticks** — posture, gait and speed limit only, and it never takes the stick; landscape takes it and keeps just the sticks and three posture keys. See `docs/design-system.md` §5B.
